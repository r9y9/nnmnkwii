...
class _JVSBaseDataSource(FileDataSource):
    def __init__(self, data_root, speakers, categories, labelmap, max_files):
        ...
        # some of the text in nonpara data have no corresponding wav file, so we need to record it
        ...
        for _, speaker in enumerate(self.speakers):
            ...
            for folder in self._folders:
                assert folder in folders,\
                    "Can not find {} for {} in its directory {}"\
                    .format(folder, speaker, speaker_folder)
            ...
            for folder in self._folders:
                ...
                if folder == "nonpara30":
                    ...
                    assert (len(txtfiles) - len(nonpara_without_wav[speaker])) == len(wavfiles)
                # strict mode, check whether each text has a corresponding wavfile, if not, report it.
                if strict:
                    with open(speaker_textfile, encoding='utf8') as file_:
                    ...
                    for line in txtlines:
                        if line + '.wav' not in wavlines:
                            print("Wavfile {} of speaker {} in {} doesn't exists."
    ...
    def collect_files(self, is_wav):
        ...
        for idx, speaker in enumerate(self.speakers):
            ...
            def read_text(filepath, nonpara, para):
                ...
                if nonpara:
                    lines = [line for line in lines if line[0] not in self._nonpara_without_wav[speaker]]
                    ...
                    if is_wav:
                        ...
                        files.extend(
                            read_text(
                                ...
                                name == "nonpara30", name == "parallel100"))
